...
//This data is always available (431860 from F18.EXE)
...
//4301E8
const char SecretHashValues[0x104] = {
	0x9A, 0x09, 0x58, 0x53, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93,
	0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93,
	0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93,
	0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93,
	0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93,
	0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93, 0x40, 0xF1, 0xF2, 0x93,
...
//InjectedFunction = 0x407A20 (this is copied)


//Original data from 4176A6 in F18.EXE (hidden function data)
const int f176A6_SIZE = 0x20;
const char f_176A6Data[f176A6_SIZE] = {
	0x36, 0x1A, 0x8C, 0x34, 0xEE, 0x29, 0x09, 0x76, 0x36, 0x50, 0x8A, 0xE2, 0xEE, 0x73, 0xE2, 0xF1,
	0x36, 0x86, 0x39, 0x7B, 0xEE, 0xBD, 0x4A, 0x3E, 0x36, 0xBC, 0x19, 0x18, 0xEE, 0x07, 0xC2, 0x21
};

//TODO start with decrypted and follow the chain (just like injected hash values)
const char F_176A6_Decrypted[0x20] = {
	0x21, 0x1A, 0x8C, 0x34, 0xF9, 0x29, 0x09, 0x76,
	0x21, 0x50, 0x8A, 0xE2, 0xBB, 0xAD, 0xDE, 0xF1,
	0x21, 0x86, 0x39, 0x7B, 0xF9, 0xBD, 0x4A, 0x3E,
	0x21, 0x3D, 0x36, 0xFA, 0xCB, 0x44, 0xBB, 0xFD
};
...
//ModifyInjectedHashValues
void XORValues(unsigned int* data, unsigned int modifyValue, ModifyType type)
{
	for(int i = 0; i < 4; ++i)
	{
		unsigned int value;		
...
void Inject(PROCESS_INFORMATION& info, bool debug)
{
	...
	//Spill-Over after InjectedFunction - first 4 bytes of StartGame (programmer mistake?)
	//A1 64 07 43 00 = mov eax, dword ptr [0x430764]
	//This happens during the 0x84 byte copy starting at txt:004079A0 InjectedStack_LoadLibraryA
	unsigned char spillOver[4] = { 0xA1, 0x64, 0x07, 0x43 };
	memcpy(&memory[index], spillOver, sizeof(spillOver));
	...
	const int spillOver_offset = arg_offset - 0x4;

	//wShowWindowResult is actually bCmapiengVxD
	...
	//Data Setup
	//More difficult because we need calculated hashes...
	unsigned int CalculatedHashValue = 0xF2639D28;

	//int f176A6Values[f176A6_SIZE];
	//memcpy(f176A6Values, f_176A6Data, f176A6_SIZE);

	unsigned int InjectedHashValues[sizeof(SecretHashValues)];
	memcpy(InjectedHashValues, SecretHashValues, sizeof(SecretHashValues));

	//IntenseCheckingInner_1
	XORValues(InjectedHashValues, 0xE094FEAC, ModifyType::XOR);
	XORValues(InjectedHashValues, 0x9827FA3E, ModifyType::XOR);
	XORValues(InjectedHashValues, 0x5216DEAD, ModifyType::XOR);

	//IntenseCheckingInner_2
	XORValues(InjectedHashValues, 0x3EABAEFC, ModifyType::XOR);

	//IntenseCheckingInner_1
	XORValues(InjectedHashValues, 0x42A2363D, ModifyType::XOR);	
	
	//SafeDiscMain
	XORValues(InjectedHashValues, CalculatedHashValue, ModifyType::XOR);

	//Inject
	XORValues(InjectedHashValues, (time.wDay & 0xFFFF), ModifyType::XOR);
	for(int i = 0; i < 16; ++i)
	{
		if((i % 0x10) == 0)
			printf("Secret Hash Values: ");
		printf("%02X ", ((unsigned char*)InjectedHashValues)[i] & 0xFF);
		if(((i + 1) % 0x10) == 0)
			printf("\n");
	}
	printf("\n");
	memcpy(&memory[arg_offset], &InjectedHashValues, 16);
